		...
		<div class="initiating">
			<div class="initiating-t">
				...
				<p class="initiating-t-time" v-if="LinkaAppointsDetailData.topic">
					...
					<span class="initiating-t-time-remove" @click="refund">申请退款</span>
				</p>
			...
			<div class="initiating-ass">
				<p>1、您的评分将不会展示给大咖</p>
				<p>2、您的评分将会按照一定规则折算到大咖的评分中，请按照真实情况填写</p>
			</div>
			<div class="initiating-xing">
				<p><span>行家专业度</span><yd-rate slot="left" v-model="postData.star" color="#D6D6D6" active-color="#ffd600" size="18px"></yd-rate></p>
				<p><span>内容满意度</span><yd-rate slot="left" v-model="postData.degree" color="#D6D6D6" active-color="#ffd600" size="18px"></yd-rate></p>
			</div>
			<div class="initiating-a">
				<p class="initiating-a-title">内容回顾</p>
				<div class="initiating-a-content">
					<textarea style="padding:3px 8px;" placeholder="(非必填)谈一谈您对本次服务的感受..." maxlength="300" v-model="postData.content"></textarea>
					<span><font color="#696969">{{postData.content.length}}</font><font color="#b0b0b0">/300</font></span>
				</div>
			</div>
			<div style="height: 1.1rem;"></div>
			<div class="initiating-assess-btn" @click="postNext">提交</div>
		</div>
	</div>
</template>
<script>
	...
	export default {
		...
		data() {
			return {
				postData: {
					...
					star: 0,
					degree: 0,
					appoint_id: this.$route.params.id
				},
				loading: false,
				title: '约见详情',
				link: 'userMake'
		...
		methods: {
			...
			async postNext() {
				...
				this.loading = true
				await api.postAppointAppraises(this.postData)
				await this.getData()
				this.loading = false
			}
		},
