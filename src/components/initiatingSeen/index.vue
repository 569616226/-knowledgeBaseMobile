		...
		<div class="initiating">
			...
			<div class="initiating-pay" v-if="LinkaAppointsDetailData.topic">
				<div class="initiating-pay-t">
					...
					<p style="font-size: .3rem;margin-top: .2rem;">您的
				...
				<div class="initiating-pay-check" v-if="LinkaAppointsDetailData.ser_type == '线下约见'">
					<div class="initiating-pay-check-list" v-for="item,index in LinkaAppointsDetailData.appoint_cases" :key="index" v-if="item.id == LinkaAppointsDetailData.case_id">
						<div class="initiating-pay-check-list-r">
							<p>
								<font color="#999999">
									<clock class="initiating-t-time-clock" fillColor="#999999" />时间：
								</font>
								{{item.appoint_time}}
							</p>
							<p style="margin-top: .2rem;">
								<font color="#999999">
									<map-marker class="initiating-t-time-clock" fillColor="#999999" />地点：
								</font>
								{{item.location}}
							</p>
						</div>
			...
			<div class="initiating-pay-flex">
				...
				<div class="initiating-pay-flex-r" @click="link_daka">
					联系大咖
				</div>
			</div>
			<transition name="fade">
				<div class="initiating-resh" v-if="removeShow">
					...
					<div class="initiating-resh-b" v-if="payShow">
						...
						<div class="initiating-resh-b-list">
							<div class="initiating-resh-b-list-p">由于本次取消预约是由您单方面提出取消，作为赔偿，您支付预约金额的{{SettingsRatio}}%作为违约金赔偿给大咖</div>
...
<script>
	...
	export default {
		...
		methods: {
			...
			seed_daka() {
				console.log(this.LinkaAppointsDetailData.guest_id)
				this.$router.push({
					name: 'privatecontent',
					params: {
						id: this.LinkaAppointsDetailData.linka_id
					}
				})
			},
			check_resh(item) {
			...
			link_daka() {
				const phone = this.LinkaAppointsDetailData.topic.data.guest_phone
