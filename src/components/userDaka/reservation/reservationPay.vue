		...
		<div class="initiating">
			...
			<div class="initiating-pay" v-if="LinkaAppointsDetailData.topic">
				<div class="initiating-pay-t">
					...
					<p style="font-size: .3rem;margin-top: .2rem;" v-if="LinkaAppointsDetailData.topic.data.ser_type == '线下约见'">请大咖<font color="#1da4ff">填写时间/地点</font>供学员选择</p>
					<p style="font-size: .3rem;margin-top: .2rem;" v-else>请大咖<font color="#1da4ff">填写时间</font>供学员选择</p>
				</div>
				<div class="initiating-pay-check" v-if="LinkaAppointsDetailData.ser_type == '线下约见'">
					...
					<div class="initiating-pay-check-set">
						<router-link :to="{name: 'reservationTime'}">
							<p class="initiating-pay-check-set-p">设置约见时间地点</p>
						</router-link>
					</div>
				</div>
				<div class="initiating-pay-check" v-else>
					...
					<div class="initiating-pay-check-set">
						<router-link :to="{name: 'reservationTime'}">
							<p class="initiating-pay-check-set-p">设置约见时间</p>
						</router-link>
					</div>
				</div>
			</div>
			...
			<div class="initiating-pay-btn" @click="getPaywait">提交</div>
		</div>
	</div>
</template>
<script>
	...
	export default {
		...
		methods: {
			...
			async getPaywait () {
				if(this.LinkaAppointsDetailData.appoint_cases.length == 0) {
					this.$dialog.toast({
						mes: '请填写方案',
						icon: 'error',
						timeout: 1000
				...
				this.loading = true
				await api.postAppointsChangeStatus(this.postPay)
				this.loading = false
				this.$router.push({
