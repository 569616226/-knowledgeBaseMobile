<template>
	<div v-title data-title="详细资料" v-loading="loading">
		...
		<div class="basicData-btn" @click="dataSave">
			<span>提交</span>
		</div>
		...
		methods: {
			...
			async upload(event) {
				if(event.target.files.length > 0) {
					...
					await this.getImgapi(this.files)
					this.imgDataUrl = this.basicimg.linkurl
				}
			},
			basicData_a() {
				this.$router.push({
			...
			async dataSave(){
				let form = {
					content: this.LinkaProfileData.profile,
					id: this.LinkaProfileData.id
				}
				this.loading = true
				await api.putGuests(form)
				this.loading = false
				this.$router.push({
					name: 'basicSuccess'
				})
