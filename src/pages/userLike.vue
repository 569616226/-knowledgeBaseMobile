<template>
	<div v-title data-title="我喜欢的大咖" v-loading="loading">
		<div class="userMake">
			<scroll ref="scroll" :data="GuestLikeLinkasData.data" :listenScrollEnd="true" :pullUpLoad="pullUpLoad" @pullingUp="onloadrefresh">
				<div class="userMake-list" v-for="item,index in GuestLikeLinkasData.data" :key="index">
					<div @click="next(item.id)">
					...
					<div class="userMake-list-r-time-lv" @click="del(item, index)">
						<delete-like fillColor="#ADADAD" width="16" height="16" />
					</div>
					<div class="clear"></div>
				</div>
			</scroll>
			<div class="userMake-noData" v-if="GuestLikeLinkasData.data.length == 0">
...
<script>
	...
	export default {
		...
		methods: {
			...
			async getData() {
				if(this.GuestLikeLinkasData.data.length == 0) {
					this.loading = true
				}
				this.GuestLikeLinkasData.page = 1
				await this.getGuestLikeLinkas(true)
				this.loading = false
			},
			async onloadrefresh() {
				await this.getGuestLikeLinkas()
			},
			next(id) {
				this.$router.push({
					name: 'dakadetail',
				...
				this.$dialog.confirm({
					...
					opts: [{
						...
						},
						{
							...
							color: 'rgb(3, 155, 229)',
							callback: () => {
								api.postLikeLinka(item.real_id)
								this.GuestLikeLinkasData.data.splice(index, 1)
							}
						}
					]
				})
